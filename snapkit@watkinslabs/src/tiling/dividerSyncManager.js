...
export class DividerSyncManager {
    ...
    _onGrabOpBegin(display, window, op) {
        ...
        if (!windowInfo) {
            this._logger.debug(`SnapKit DEBUG: DividerSyncManager - resize on UNTRACKED window, ignoring`);
        ...
        this._logger.debug(`SnapKit DEBUG: DividerSyncManager - resize STARTED on tracked window`);
        this._logger.debug(`SnapKit DEBUG: - windowTitle: ${window.get_title()}`);
        this._logger.debug(`SnapKit DEBUG: - layoutId from WindowTracker: ${windowInfo.layoutId}`);
        this._logger.debug(`SnapKit DEBUG: - zoneIndex: ${windowInfo.zoneIndex}`);
        this._logger.debug(`SnapKit DEBUG: - monitorIndex: ${windowInfo.monitorIndex}`);
    ...
    _onGrabOpEnd(display, window, op) {
        ...
        try {
            ...
            this._logger.debug(`SnapKit DEBUG: - Current overrides: ${JSON.stringify(overrides)}`);
            ...
            this._logger.debug(`SnapKit DEBUG: - Resolved ${zones.length} zones`);
    ...
    _calculateDividerUpdates(oldRect, newRect, zone, workArea, op, layout, overrides = []) {
        ...
        this._logger.debug(`SnapKit DEBUG: _calculateDividerUpdates - deltas: left=${deltaLeft}, right=${deltaRight}, top=${deltaTop}, bottom=${deltaBottom}`);
        ...
        this._logger.debug(`SnapKit DEBUG: _calculateDividerUpdates - edges moved: ${edgesToCheck.map(e => e.edge).join(', ')}`);
        ...
        for (const { edge, newPos } of edgesToCheck) {
            ...
            if (!dividerInfo) {
                this._logger.debug(`SnapKit DEBUG: _calculateDividerUpdates - no divider found for edge=${edge}`);
                continue;
            }
            ...
            this._logger.debug(`SnapKit DEBUG: _calculateDividerUpdates - edge=${edge}, path="${dividerInfo.path}", newRatio=${newRatio.toFixed(3)}`);
    ...
    _findDividerForEdgeWithArea(edge, zone, workArea, layout, overrides = []) {
        ...
        this._logger.debug(`SnapKit DEBUG: _findDividerForEdgeWithArea - looking for ${requiredDirection} divider at edge=${edge}, edgePos=${zoneEdgePos}`);
        ...
        if (result) {
            this._logger.debug(`SnapKit DEBUG: _findDividerForEdgeWithArea - found divider at path="${result.path}" for edge=${edge}`);
        } else {
            this._logger.debug(`SnapKit DEBUG: _findDividerForEdgeWithArea - no divider found for edge=${edge}`);
        }
    ...
    _findDividerInTreeWithArea(node, path, area, zoneIndex, edge, edgePos, requiredDirection, overrideMap) {
        ...
        if (direction === requiredDirection) {
            ...
            this._logger.debug(`SnapKit DEBUG: _findDividerInTreeWithArea - checking path="${path}", dividerPos=${dividerPos.toFixed(0)}, edgePos=${edgePos}, diff=${Math.abs(edgePos - dividerPos).toFixed(0)}`);
    ...
    _resnapAllWindows(monitorIndex, layoutId, layout) {
        ...
        this._logger.debug(`SnapKit DEBUG: _resnapAllWindows - found ${windows.length} windows in layout ${layoutId} on monitor ${monitorIndex}`);
        this._logger.debug(`SnapKit DEBUG: _resnapAllWindows - using overrides: ${JSON.stringify(overrides)}`);

        for (const window of windows) {
            const info = this._windowTracker.getWindowInfo(window);
            if (info) {
                this._logger.debug(`SnapKit DEBUG: _resnapAllWindows - re-snapping "${window.get_title()}" to zone ${info.zoneIndex}`);
