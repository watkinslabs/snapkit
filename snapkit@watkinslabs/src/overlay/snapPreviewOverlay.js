...
export class SnapPreviewOverlay extends BaseOverlay {
    ...
    showPreview(monitorIndex, layout, options = {}) {
        ...
        // Debug logging
        this._logger.debug(`SnapKit DEBUG: SnapPreviewOverlay.showPreview - monitorIndex=${monitorIndex}`);
        this._logger.debug(`SnapKit DEBUG: SnapPreviewOverlay.showPreview - layout.id=${layout?.id}, layout.name=${layout?.name}`);
        ...
        try {
            ...
            this._logger.debug(`SnapKit DEBUG: SnapPreviewOverlay - resolved ${this._zones.length} zones`);
    ...
    _canWindowFitInZone(window, zone) {
        if (!window) {
            this._logger.debug('SnapKit DEBUG: No window provided for size check');
            return true;
        }
        ...
        try {
            ...
            this._logger.debug(`SnapKit DEBUG: Checking window fit: "${windowTitle}"`);
            this._logger.debug(`  WM Class: ${wmClass}`);
            this._logger.debug(`  Window Type: ${windowType}`);
            this._logger.debug(`  Client Type: ${clientType} (0=X11, 1=Wayland)`);
            this._logger.debug(`  Zone: ${zone.width}x${zone.height}`);
            this._logger.debug(`  Window frame rect: ${rect.width}x${rect.height}`);
            this._logger.debug(`  Window buffer rect: ${bufferRect.width}x${bufferRect.height}`);
            this._logger.debug(`  Allows resize: ${allowsResize}`);
            this._logger.debug(`  Allows move: ${window.allows_move ? window.allows_move() : 'unknown'}`);
            this._logger.debug(`  Can maximize: ${window.can_maximize ? window.can_maximize() : 'unknown'}`);
            this._logger.debug(`  Is override redirect: ${window.is_override_redirect ? window.is_override_redirect() : 'unknown'}`);
            ...
            this._logger.debug(`  Available size methods:`);
            this._logger.debug(`    - get_minimum_size_hints: ${typeof window.get_minimum_size_hints === 'function' ? 'YES' : 'NO'}`);
            this._logger.debug(`    - get_maximum_size_hints: ${typeof window.get_maximum_size_hints === 'function' ? 'YES' : 'NO'}`);
            this._logger.debug(`    - get_size_hints: ${typeof window.get_size_hints === 'function' ? 'YES' : 'NO'}`);
            this._logger.debug(`    - get_mutter_hints: ${typeof window.get_mutter_hints === 'function' ? 'YES' : 'NO'}`);

            // Try get_minimum_size_hints()
            if (typeof window.get_minimum_size_hints === 'function') {
                try {
                    const minHints = window.get_minimum_size_hints();
                    this._logger.debug(`  get_minimum_size_hints() returned:`, minHints);
                    ...
                    if (minHints && Array.isArray(minHints) && minHints.length === 2) {
                        ...
                        this._logger.debug(`  Parsed minimum size: ${minWidth || 'none'}x${minHeight || 'none'}`);
                    } else {
                        this._logger.debug(`  Minimum size hints: not available or invalid format`);
                    }
                } catch (e) {
                    this._logger.debug(`  Error getting minimum size hints: ${e.message}`);
                }
            }
            ...
            if (typeof window.get_maximum_size_hints === 'function') {
                try {
                    ...
                    this._logger.debug(`  get_maximum_size_hints() returned:`, maxHints);
                    ...
                    if (maxHints && Array.isArray(maxHints) && maxHints.length === 2) {
                        ...
                        this._logger.debug(`  Parsed maximum size: ${maxWidth || 'none'}x${maxHeight || 'none'}`);
                    }
                } catch (e) {
                    this._logger.debug(`  Error getting maximum size hints: ${e.message}`);
                }
            }
            ...
            if (typeof window.get_size_hints === 'function') {
                try {
                    ...
                    this._logger.debug(`  get_size_hints() returned:`, hints);
                } catch (e) {
                    this._logger.debug(`  Error getting size hints: ${e.message}`);
                }
            }
            ...
            if (typeof window.get_mutter_hints === 'function') {
                try {
                    ...
                    this._logger.debug(`  get_mutter_hints() returned:`, mutterHints);
                } catch (e) {
                    this._logger.debug(`  Error getting mutter hints: ${e.message}`);
                }
            }
            ...
            if (!allowsResize) {
                ...
                this._logger.debug(`  Fixed-size window - Using current size as minimum: ${effectiveMinWidth}x${effectiveMinHeight}`);
                ...
                this._logger.debug(`  Resizable with hints - Effective minimum: ${effectiveMinWidth}x${effectiveMinHeight}`);
                this._logger.debug(`    (min_width=${minWidth || 'unset'}, min_height=${minHeight || 'unset'})`);
            } else {
                // Resizable without hints: use current size as conservative estimate
                effectiveMinWidth = rect.width;
                effectiveMinHeight = rect.height;
                this._logger.debug(`  Resizable without hints - Using current size as estimate: ${effectiveMinWidth}x${effectiveMinHeight}`);
            }

            // Check if window can fit
            const canFit = zone.width >= effectiveMinWidth && zone.height >= effectiveMinHeight;
            this._logger.debug(`  Final validation: Zone ${zone.width}x${zone.height} vs Effective Min ${effectiveMinWidth}x${effectiveMinHeight} => Can fit: ${canFit}`);
            ...
            this._logger.debug('SnapKit DEBUG: Error checking window fit', error);
    ...
    _clearWindowPreview() {
        if (this._previewActor) {
            try {
                if (!this._previewActor.is_destroyed?.()) {
    ...
    _clearZoneActors() {
        for (const zoneData of this._zoneActors) {
            if (zoneData.actor && !zoneData.actor.is_destroyed?.()) {
                try {
