...
export class LayoutOverlayAnimation {
    ...
    flash(actor, color = 'white', duration = 300) {
        ...
        const timeoutId = GLib.timeout_add(GLib.PRIORITY_DEFAULT, duration, () => {
    ...
    stagger(actors, animFunc, staggerDelay = 50) {
        actors.forEach((actor, index) => {
            const timeoutId = GLib.timeout_add(GLib.PRIORITY_DEFAULT, index * staggerDelay, () => {
                this._pendingTimeouts.delete(timeoutId);
                if (actor && actor.get_stage()) {
                    animFunc(actor);
                }
