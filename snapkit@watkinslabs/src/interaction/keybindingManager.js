...
export class KeybindingManager {
    ...
    initialize() {
        ...
        try {
            ...
            this._registerKeybinding('move-window-zone-left', () => {
                this._onMoveWindowZone('left');
            });

            this._registerKeybinding('move-window-zone-right', () => {
                this._onMoveWindowZone('right');
            });

            this._registerKeybinding('move-window-zone-up', () => {
                this._onMoveWindowZone('up');
            });

            this._registerKeybinding('move-window-zone-down', () => {
                this._onMoveWindowZone('down');
            });

            this._registerKeybinding('move-window-zone-next', () => {
                this._onMoveWindowZone('next');
            });

            this._registerKeybinding('move-window-zone-prev', () => {
                this._onMoveWindowZone('prev');
            });

            for (let zone = 1; zone <= 9; zone++) {
                this._registerKeybinding(`move-window-zone-${zone}`, () => {
                    this._onMoveWindowZone(`index:${zone}`);
                });
            }
    ...
    /**
     * Handle move-window-zone keybinding
     * @private
     * @param {string} command
     */
    _onMoveWindowZone(command) {
        const window = global.display.focus_window;
        if (!window) {
            this._logger.debug('No focused window for zone move');
            return;
        }

        this._eventBus.emit('keyboard-move-window-zone', {
            window,
            command
        });
    }
